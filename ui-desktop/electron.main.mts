...
function createMainWindow() {
  const mainWindowState = windowStateKeeper({ defaultWidth: INITIAL_WIDTH, defaultHeight: INITIAL_HEIGHT });
...
app.whenReady().then(() => {
  if (process.env.NODE_ENV === 'development') {
    try { startUiDevSocket(); } catch (e) { console.error('[UI-DEV] 启动失败', e); }
  }
  createMainWindow();
...
// Dev command 转发到渲染进程
ipcMain.on('control:dev-command', (_event, cmd) => {
  const all = BrowserWindow.getAllWindows();
  for (const win of all) {
    win.webContents.send('control:dev-command', cmd);
  }
});
